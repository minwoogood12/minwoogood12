...
#define _DUTY_NEU 1245  // [2952] 서보의 중간 각도값
#define _DUTY_MAX 2400  // [1691] 서보의 최대 각도

// Servo speed control
#define _SERVO_ANGLE 30  //[2967] 서보 각도 설정
#define _SERVO_SPEED 150//[2959] 서보의 속도 설정
...
////////////////////
  ...
  // PID control logic
    ...
    control = pterm + dterm;                // [2959] 중간에서 얼마나 움직일 것인가

    dterm = _KD * (error_curr - error_prev );
    
  // duty_target = f(duty_neutral, control)
    duty_target = _DUTY_NEU + control; //[2959] 목적지에서의 거리에 비례하여 목표 각도 변경
  ...
  } //....[2955] 범위 제어를 통해 서보암의 움직임이 예상을 벗어나 제어가 되지 않는 경우 제거
  ...
  if(event_serial) {
    ...
    Serial.print(",pterm:");
    Serial.print(map(pterm, -1000, 1000, 501, 610));    // [2961] 비례제어량 출력
    Serial.print(",dterm:"); 
    Serial.print(map(dterm, -1000, 1000, 510, 610)); // [2957] 목표로 하는 거리 출력
    Serial.print(",duty_target:"); 
    Serial.print(map(duty_target, 1000, 2000, 410, 510)); // [2957] 서보모터에 입력한 값 출력
    Serial.print(",duty_curr:");
    Serial.print(map(duty_curr, 1000, 2000, 410, 510));
    Serial.println(",Min:100,Low:200,dis_target:255,High:310, Max:410");
  }
}
...
float ir_distance_filtered(void){ // return value unit: mm
  ...
  float val = ir_distance();        // [2961] val -> float val
  return 100 + 300.0 / (b - a) * (val - a); //[2959] 100mm일때 측정값과 400mm일때 측정값
                                            //       기준으로 거리 보정
}
